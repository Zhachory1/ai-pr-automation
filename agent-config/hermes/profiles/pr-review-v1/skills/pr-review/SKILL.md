...
**Human owns merge.** This profile submits the verdict-appropriate GitHub review but never merges. Accountable merge remains human-owned.
...
- **Verdict mapping:** `approve` and `approve-with-nits` submit `APPROVE`; `needs-info` submits `COMMENT`; `request-changes` and `block` submit `REQUEST_CHANGES`.
- **No findings:** submit `APPROVE` pinned to the reviewed head.
...
**Publish feedback.** In `post` mode, re-check the head SHA, compute/check the idempotency marker, preview the exact body/payload, then submit one verdict-appropriate GitHub review pinned with `commit_id`. In `report-only` mode, state that nothing was posted.
