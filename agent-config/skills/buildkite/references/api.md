---
name: buildkite
description: "Query the Buildkite REST API using BUILDKITE_API_TOKEN (bearer auth) against https://api.buildkite.com/v2. Use for investigating build failures, pulling build/job logs, listing pipelines, downloading artifacts, retrying jobs, and debugging CI issues."
---
...
- **Overview** — auth, base URL, URL pattern convention
- **API Reference** — Pipelines, Builds (query params, states, response fields), Jobs (logs, env, retry), Annotations, Artifacts, Agents
- **Common Workflows** — investigate failures, find recent failures, pull logs, download artifacts, retry/rebuild, env vars, find by commit, running builds, unblock
- **Debugging Workflow** — step-by-step failure investigation sequence
- **Parsing Build URLs** — extract org/pipeline/build from Buildkite URLs
- **Notes** — pagination, rate limits, token scopes

## Overview

Use the Buildkite REST API to investigate builds, pull logs, identify failures, download artifacts, and manage pipelines. Authentication is via the `BUILDKITE_API_TOKEN` environment variable as a Bearer token.
