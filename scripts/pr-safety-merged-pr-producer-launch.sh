...
export PATH="${PR_SAFETY_MERGED_PR_PATH:-/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin}"
ENV_FILE="${PR_SAFETY_MERGED_PR_ENV_FILE:-$ROOT/.env}"
PRODUCER="${PR_SAFETY_MERGED_PR_PRODUCER_BIN:-$ROOT/bin/pr-safety-merged-pr-producer}"
...
: "${PR_SAFETY_MERGED_PR_AUTHORS:?set PR_SAFETY_MERGED_PR_AUTHORS}"
...
: "${GH_TOKEN:?set GH_TOKEN (SAML-authorized for the org)}"
command -v gh >/dev/null || { echo "missing gh" >&2; exit 2; }

lock_dir="${PR_SAFETY_MERGED_PR_LOCK_DIR:-$HOME/.local/state/agent-fleet}"
mkdir -p "$lock_dir"
chmod 700 "$lock_dir"
...
try:
    ...
    raise SystemExit(0)
os.execvp(command[0], command)' "$lock_dir/pr-safety-merged-pr-producer.lock" "$PRODUCER"
