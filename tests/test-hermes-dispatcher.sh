...
# observe the peak concurrency. Assert pr-maintain peaks at exactly its cap (3), review/swe at 1.
set -euo pipefail
cd "$(dirname "$0")/.."
tmp="$(mktemp -d)"; trap 'rm -rf "$tmp"; [[ -n "${disp:-}" ]] && kill "$disp" 2>/dev/null || true' EXIT
mkdir -p "$tmp/bin" "$tmp/libexec" "$tmp/run"

# Fake psql: hermes_queue_depth('<kind>') -> 99 for maintain/review/swe, 0 for memory-curate.
cat > "$tmp/bin/psql" <<'SH'
#!/usr/bin/env bash
args="$*"
...
chmod +x "$tmp/libexec/hermes-queue-runner" "$tmp/libexec/hermes-memory-curate"
