...
apply_upgrades() { docker exec "$CID" sh -c 'for f in /migrations/0[2-9]-*.sql /migrations/1[0-9]-*.sql; do psql -U fleet -d fleet -v ON_ERROR_STOP=1 -f "$f" >/dev/null 2>&1 || exit 1; done'; }
...
docker exec "$CID" sh -c 'for f in /migrations/0[1-9]-*.sql /migrations/1[0-9]-*.sql; do psql -U fleet -d fresh -v ON_ERROR_STOP=1 -f "$f" >/dev/null 2>&1 || exit 1; done'
