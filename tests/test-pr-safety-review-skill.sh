...
check "$skill" "Use the \`hindsight\` MCP server to recall prior context and to retain durable, non-sensitive findings" "scoped memory-retain"
check "$skill" "bound to the \`pr-safety\` bank endpoint, so retain reaches only the \`pr-safety\` bank" "memory-bank isolation"
