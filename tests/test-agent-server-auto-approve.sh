...
if [[ "$1 $2" == "api user" ]]; then
  ...
  if [[ "$*" == *"--jq .headRefOid"* ]]; then echo "$head"; else printf '{"baseRefOid":"%s","headRefOid":"%s","author":{"login":"%s"}}\n' "$(base_for)" "$head" "$author"; fi
  ...
  if [[ "$*" == *'[.user.login, .state, .commit_id'* ]]; then
    printf 'reviewer-bot\t%s\t%s\t<!-- ai-pr-automation head=%s -->\n' "$state" "$head" "$head"
  ...
  printf '%s' "${GH_TOKEN:-}" > "$TEST_STATE/publisher-token-$pr"
  ...
  printf '%s\n' "$*" > "$TEST_STATE/maintain-review-call"
else
...
[[ "$GH_TOKEN" == readonly-token && "$GITHUB_TOKEN" == readonly-token ]]
printf '%s\n' "$PR_NUMBER" >> "$TEST_STATE/runner-prs"
if gh pr review "$PR_NUMBER" -R "$PR_REPO" --comment 2> "$TEST_STATE/shim-comment-$PR_NUMBER"; then exit 4; fi
grep -Fq 'agent-server owns GitHub reviews' "$TEST_STATE/shim-comment-$PR_NUMBER"
...
export GH_TOKEN=publisher-token AGENT_SERVER_KIND=pr-review AGENT_SERVER_REVIEW_RUNTIME=legacy
export AGENT_SERVER_REVIEW_CHILD_GH_TOKEN=readonly-token AGENT_SERVER_RUNNER="$TMP/bin/runner" AGENT_SERVER_WORK_ROOT="$TMP/work"
...
[[ "$(cat "$TMP/posts-7")" == 1 ]]
[[ "$(cat "$TMP/publisher-token-7")" == publisher-token ]]
...
[[ "$(q "SELECT status FROM requests WHERE dedupe_key LIKE 'owner/repo#12@%';")" == reconcile ]]
[[ ! -e "$TMP/posts-8" && ! -e "$TMP/posts-9" && ! -e "$TMP/posts-10" && ! -e "$TMP/posts-11" && ! -e "$TMP/posts-12" ]]
[[ "$(sort -n "$TMP/runner-prs" | tr '\n' ' ')" == "7 8 9 11 12 " ]]

awk '/^setup_github_lease_shim\(\) \{/{p=1} p{print} p&&/^}/{exit}' bin/agent-server > "$TMP/setup-shim.sh"
# shellcheck disable=SC1090
. "$TMP/setup-shim.sh"
TIMEOUT_BIN="$(command -v timeout || command -v gtimeout)"
LEASE_DB_TIMEOUT=3
maintain_shim="$(setup_github_lease_shim "$TMP/maintain-shim" 1 nonce false)"
printf '#!/bin/sh\nexit 0\n' > "$maintain_shim/lease-check"
chmod +x "$maintain_shim/lease-check"
"$maintain_shim/gh" pr review 7 --comment
[[ "$(cat "$TMP/maintain-review-call")" == "pr review 7 --comment" ]]

echo "PASS: agent-server validates reviews, isolates child token, and preserves maintain review egress"
