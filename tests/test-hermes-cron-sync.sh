...
grep -q 'hermes-memory-curate --enqueue' "$tmp/hermes/scripts/ai-pr-automation-memory-curate.sh"
# memory-curate self-triggers via the executor's own --enqueue; repo roles do not self-enqueue
if grep -Eq 'enqueue' "$tmp/hermes/scripts/ai-pr-automation-pr-review.sh"; then
