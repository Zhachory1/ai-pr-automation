...
export GOOGLE_CHAT_ACCESS_TOKEN=fixture-token GOOGLE_CHAT_SPACE=spaces/AAAA PR_SAFETY_CHAT_SENDERS=users/alice
...
mkdir -p "$TMP/bin"; ln -s "$PWD/tests/fake-pr-safety-chat-gh.sh" "$TMP/bin/gh"
export PATH="$TMP/bin:$PATH"
chmod +x tests/fake-pr-safety-chat-gh.sh bin/pr-safety-chat-producer
...
check "bot non-allowlisted free-form and injected content are ignored" "q \"SELECT count(*) FROM requests WHERE kind='pr-safety-review';\" | grep -qx 1 && q \"SELECT count(*) FROM pr_safety_chat_events;\" | grep -qx 1 && [[ ! -e \"$MARKER\" ]]"
