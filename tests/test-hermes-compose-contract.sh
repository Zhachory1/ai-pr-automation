...
jq -e '
  ...
  ($h.environment | keys == ["API_SERVER_ENABLED", "API_SERVER_HOST", "API_SERVER_KEY", "API_SERVER_PORT", "HERMES_API_CALL_STALE_TIMEOUT", "HERMES_CODEX_EVENT_STALE_TIMEOUT_SECONDS", "HERMES_IGNORE_RULES", "HERMES_SAFE_MODE", "HTTPS_PROXY", "HTTP_PROXY", "NO_PROXY", "OPENAI_API_KEY", "OPENAI_BASE_URL", "OPENSSL_CONF"]) and
  (($h | has("ports")) | not) and
