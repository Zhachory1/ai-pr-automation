#!/usr/bin/env bash
# Tests the forward-fix-server queue worker end-to-end against ephemeral Postgres, with a FAKE
...
case "$(cat "$FAKE_HARNESS_MODE" 2>/dev/null)" in
  fail) echo "forward-fix: clone failed for X" >&2; exit 2 ;;
  nopr) echo "committed on branch (--no-pr)"; exit 0 ;;
  *)    echo "2026-01-01 forward-fix draft PR: https://github.com/ROKT/x/pull/99"; exit 0 ;;
esac
SH
chmod +x "$TMP/fake-harness"
export FORWARD_FIX_HARNESS="$TMP/fake-harness" FAKE_HARNESS_ARGS="$TMP/args" FAKE_HARNESS_MODE="$TMP/mode"
export FORWARD_FIX_POLL_INTERVAL=1

# run the server for one claim cycle in the background, stop after the row leaves 'queued'
run_one() {
  ( timeout 30 bash bin/forward-fix-server >/"$TMP"/server.log 2>&1 & echo $! > "$TMP/srv.pid" )
  for _ in $(seq 1 25); do
    local st; st="$(q "SELECT status FROM requests ORDER BY id DESC LIMIT 1")"
    [[ "$st" == done || "$st" == failed ]] && break; sleep 1
  done
  kill "$(cat "$TMP/srv.pid")" 2>/dev/null || true; pkill -f forward-fix-server 2>/dev/null || true; sleep 1
}

# 1) handoff source -> --handoff <path>, success -> done + posted_ref carries the PR url
echo success > "$TMP/mode"
q "INSERT INTO requests(kind,payload,dedupe_key) VALUES('forward-fix','{\"source\":\"handoff\",\"handoff_path\":\"/h/x.md\",\"no_pr\":false}','handoff:x.md');" >/dev/null
run_one
...
echo success > "$TMP/mode"
q "INSERT INTO requests(kind,payload,dedupe_key) VALUES('forward-fix','{\"source\":\"issue\",\"issue\":\"ROKT/cpi#123\",\"no_pr\":false}','issue:ROKT/cpi#123');" >/dev/null
run_one
...
echo nopr > "$TMP/mode"
q "INSERT INTO requests(kind,payload,dedupe_key) VALUES('forward-fix','{\"source\":\"prompt\",\"repo\":\"ROKT/cpi\",\"prompt\":\"do x\",\"no_pr\":true}','prompt:ROKT/cpi:abc');" >/dev/null
run_one
...
echo fail > "$TMP/mode"
q "INSERT INTO requests(kind,payload,dedupe_key) VALUES('forward-fix','{\"source\":\"issue\",\"issue\":\"ROKT/cpi#7\",\"no_pr\":false}','issue:ROKT/cpi#7');" >/dev/null
run_one
...
rm -f "$TMP/args"
q "INSERT INTO requests(kind,payload,dedupe_key) VALUES('forward-fix','{\"source\":\"bogus\"}','bad:1');" >/dev/null
run_one
