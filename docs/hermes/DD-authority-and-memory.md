...
1. DONE: `authority.yaml` allowlist (`scripts/hermes-authority.py`, `authority.example.yaml`);
   `09-hermes-yaml-authority.sql` drops the proof param, `hermes_repository_authorized`, the
   enrollment table, and the proof column; evidence scripts removed. Tests:
   `test-hermes-queue-authority.sh`, `test-hermes-authority.sh`.
2. Superseded by step 1: there is no enrollment left to drop for memory-curate; it just needs the
   MCP rewrite below.
